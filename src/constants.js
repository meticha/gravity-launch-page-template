import Artifact from "./assets/artifact";

export const elementsConfig = [
  {
    type: "circle",
    style: {
      width: "w-8 sm:w-8 md:w-8 lg:w-8",
      height: "h-8 sm:h-8 md:h-8 lg:h-8",
      backgroundColor: "bg-red-500",
      borderRadius: "rounded-full",
      boxShadow: "shadow-md hover:shadow-lg",
      transform: "hover:scale-110",
    },
  },
  {
    type: "square",
    style: {
      width: "w-8 sm:w-8 md:w-8 lg:w-8",
      height: "h-8 sm:h-8 md:h-8 lg:h-8",
      backgroundColor: "bg-blue-500",
      borderRadius: "rounded-sm",
      boxShadow: "shadow-md hover:shadow-lg",
      transform: "hover:scale-110",
    },
  },
  {
    type: "square",
    style: {
      width: "w-8 sm:w-8 md:w-8 lg:w-8",
      height: "h-8 sm:h-8 md:h-8 lg:h-8",
      backgroundColor: "bg-[#FF5733]",
      boxShadow: "shadow-md hover:shadow-lg",
      transform: "hover:scale-110",
    },
  },
  {
    type: "square",
    style: {
      width: "w-8 sm:w-8 md:w-8 lg:w-8",
      height: "h-8 sm:h-8 md:h-8 lg:h-8",
      backgroundColor: "bg-[#33FF57]",
      boxShadow: "shadow-md hover:shadow-lg",
      transform: "hover:scale-110",
    },
  },
  {
    type: "square",
    style: {
      width: "w-8 sm:w-8 md:w-8 lg:w-8",
      height: "h-8 sm:h-8 md:h-8 lg:h-8",
      backgroundColor: "bg-[#3357FF]",
      boxShadow: "shadow-md hover:shadow-lg",
      transform: "hover:scale-110",
    },
  },
  {
    type: "square",
    style: {
      width: "w-8 sm:w-8 md:w-8 lg:w-8",
      height: "h-8 sm:h-8 md:h-8 lg:h-8",
      backgroundColor: "bg-[#FF33A1]",
      boxShadow: "shadow-md hover:shadow-lg",
      transform: "hover:scale-110",
    },
  },
  {
    type: "square",
    style: {
      width: "w-8 sm:w-8 md:w-8 lg:w-8",
      height: "h-8 sm:h-8 md:h-8 lg:h-8",
      backgroundColor: "bg-[#A133FF]",
      boxShadow: "shadow-md hover:shadow-lg",
      transform: "hover:scale-110",
    },
  },
  {
    type: "square",
    style: {
      width: "w-8 sm:w-8 md:w-8 lg:w-8",
      height: "h-8 sm:h-8 md:h-8 lg:h-8",
      backgroundColor: "bg-[#33FFF5]",
      boxShadow: "shadow-md hover:shadow-lg",
      transform: "hover:scale-110",
    },
  },
  {
    type: "square",
    style: {
      width: "w-8 sm:w-8 md:w-8 lg:w-8",
      height: "h-8 sm:h-8 md:h-8 lg:h-8",
      backgroundColor: "bg-[#F5FF33]",
      boxShadow: "shadow-md hover:shadow-lg",
      transform: "hover:scale-110",
    },
  },
  {
    type: "square",
    style: {
      width: "w-8 sm:w-8 md:w-8 lg:w-8",
      height: "h-8 sm:h-8 md:h-8 lg:h-8",
      backgroundColor: "bg-[#FF6F33]",
      boxShadow: "shadow-md hover:shadow-lg",
      transform: "hover:scale-110",
    },
  },
  {
    type: "square",
    style: {
      width: "w-8 sm:w-8 md:w-8 lg:w-8",
      height: "h-8 sm:h-8 md:h-8 lg:h-8",
      backgroundColor: "bg-[#33FF6F]",
      boxShadow: "shadow-md hover:shadow-lg",
      transform: "hover:scale-110",
    },
  },
  {
    type: "square",
    style: {
      width: "w-8 sm:w-8 md:w-8 lg:w-8",
      height: "h-8 sm:h-8 md:h-8 lg:h-8",
      backgroundColor: "bg-[#336FFF]",
      boxShadow: "shadow-md hover:shadow-lg",
      transform: "hover:scale-110",
    },
  },

  {
    type: "square",
    style: {
      width: "w-8 sm:w-8 md:w-8 lg:w-8",
      height: "h-8 sm:h-8 md:h-8 lg:h-8",
      backgroundColor: "bg-[#F533FF]",
      boxShadow: "shadow-md hover:shadow-lg",
      transform: "hover:scale-110",
    },
  },

  {
    type: "square",
    style: {
      width: "w-8 sm:w-8 md:w-8 lg:w-8",
      height: "h-8 sm:h-8 md:h-8 lg:h-8",
      backgroundColor: "bg-[#FFA133]",
      boxShadow: "shadow-md hover:shadow-lg",
      transform: "hover:scale-110",
    },
  },

  {
    type: "image",
    src: "/assets/img3.png",
    width: "w-8 md:w-16",
    // height: "h-32 md:h-64",
    imageStyle: {
      filter: "drop-shadow(0 2px 2px rgba(0, 0, 0, 0.1))",
      // transform: "hover:scale-105 transition-transform duration-300",
    },
  },
  {
    type: "image",
    src: "/assets/Group 24.svg",
    width: "w-32 md:w-40",
    // height: "h-16 md:h-24",
    imageStyle: {
      filter: "drop-shadow(0 2px 2px rgba(0, 0, 0, 0.1))",
      // transform: "hover:rotate-6 transition-transform duration-300",
    },
  },
  {
    type: "image",
    src: "/assets/Group 23.svg",
    width: "w-24 md:w-48",
    // height: "h-24 md:h-48",
    imageStyle: {
      filter: "drop-shadow(0 2px 2px rgba(0, 0, 0, 0.1))",
      // transform: "hover:scale-110 transition-transform duration-300",
    },
  },
  {
    type: "image",
    src: "/assets/Group 22.svg",
    width: "w-8 md:w-16",
    // height: "h-24 md:h-48",
    imageStyle: {
      filter: "drop-shadow(0 2px 2px rgba(0, 0, 0, 0.1))",
      // transform: "hover:-rotate-6 transition-transform duration-300",
    },
  },
  {
    type: "image",
    src: "/assets/Group 21.svg",
    width: "w-8 md:w-12",
    // height: "h-8 md:h-12",
    imageStyle: {
      filter: "drop-shadow(0 1px 1px rgba(0, 0, 0, 0.1))",
      // transform: "hover:scale-125 transition-transform duration-300",
    },
  },
  {
    type: "image",
    src: "/assets/Group 15.svg",
    width: "w-8 md:w-20",
    // height: "h-20 md:h-32",
    imageStyle: {
      filter: "drop-shadow(0 2px 2px rgba(0, 0, 0, 0.1))",
      // transform: "hover:rotate-12 transition-transform duration-300",
    },
  },
  {
    type: "image",
    src: "/assets/Group 20.svg",
    width: "w-16 md:w-24",
    // height: "h-12 md:h-20",
    imageStyle: {
      filter: "drop-shadow(0 1px 1px rgba(0, 0, 0, 0.1))",
      // transform: "hover:scale-110 transition-transform duration-300",
    },
  },
  // {
  //   type: "image",
  //   src: "/assets/Group 25.svg",
  //   width: "w-16 md:w-24",
  //   // height: "h-24 md:h-48",
  //   imageStyle: {
  //     filter: "drop-shadow(0 2px 2px rgba(0, 0, 0, 0.1))",
  //     // transform: "hover:rotate-6 transition-transform duration-300",
  //   },
  // },
  {
    type: "image",
    src: "/assets/Group 26 (2).png",
    width: "w-12 md:w-16",
    // height: "h-24 md:h-48",
    imageStyle: {
      filter: "drop-shadow(0 2px 2px rgba(0, 0, 0, 0.1))",
      // transform: "hover:rotate-6 transition-transform duration-300",
    },
  },
  {
    type: "image",
    src: "/assets/Group 27.svg",
    width: "w-16 md:w-24",
    // height: "h-24 md:h-48",
    imageStyle: {
      filter: "drop-shadow(0 2px 2px rgba(0, 0, 0, 0.1))",
      // transform: "hover:rotate-6 transition-transform duration-300",
    },
  },
  {
    type: "image",
    src: "/assets/Group 28.svg",
    width: "w-12 md:w-16",
    // height: "h-24 md:h-48",
    imageStyle: {
      filter: "drop-shadow(0 2px 2px rgba(0, 0, 0, 0.1))",
      // transform: "hover:rotate-6 transition-transform duration-300",
    },
  },
  {
    type: "image",
    src: "/assets/Group 30.svg",
    width: "w-16 md:w-24",
    // height: "h-24 md:h-48",
    imageStyle: {
      filter: "drop-shadow(0 2px 2px rgba(0, 0, 0, 0.1))",
      // transform: "hover:rotate-6 transition-transform duration-300",
    },
  },

  {
    type: "image",
    src: "/assets/Simplification (2).svg",
    width: "w-24 md:w-48",
    // height: "h-24 md:h-48",
    imageStyle: {
      filter: "drop-shadow(0 2px 2px rgba(0, 0, 0, 0.1))",
      // transform: "hover:rotate-6 transition-transform duration-300",
    },
  },
  {
    type: "image",
    src: "/assets/Simplification (3).svg",
    width: "w-16 md:w-24",
    // height: "h-24 md:h-48",
    imageStyle: {
      filter: "drop-shadow(0 2px 2px rgba(0, 0, 0, 0.1))",
      // transform: "hover:rotate-6 transition-transform duration-300",
    },
  },

  // {
  //   type: "image",
  //   src: "/assets/Simplification (1).svg",
  //   width: "w-32 md:w-64",
  //   // height: "h-32 md:h-64",
  //   isLink: true,
  //   href: "https://www.meticha.com",
  //   imageStyle: {
  //     borderRadius: "rounded-xl",
  //     border: "border-2 border-white",
  //     boxShadow: "shadow-md",
  //     // transform: "hover:scale-105 transition-transform duration-300",
  //   },
  //   containerStyle: {
  //     position: "relative",
  //   },
  // },
];
